...
function reducer(state: stateType, action: actionType) {
  switch (action.type) {
    ...
    case "loadingSelected":
      return { ...state, loadingSelected: action.payload };
    case "addToBookmarked":
      function addToBookmarkList(selected: any) {
        if (
          state.bookmarkList?.find(
            (el: selectedRecipeType) => el.id === selected.id
          )
        )
...
function RecipeProvider({ children }: any) {
  ...
  async function loadRecipes(query: string) {
    try {
      ...
      if (resJSON.results === 0)
        throw new Error(
          "Sorry we do not have a recipe for your search, Try something else. 👍"
        );
  ...
  async function deleteSelectedRecipeFromAPI(id: string) {
    try {
      dispatch({ type: "loadingSelected", payload: true });
      const res = await fetch(`${BASE_URL}/${id}?key=${KEY}`, {
        method: "DELETE",
...
function useRecipe() {
  ...
  if (context === undefined)
    throw new Error("component is used outside of context provider");
  return context;
}
